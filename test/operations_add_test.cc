...
TEST(OperationsAddTest, SimpleAdd2D)
{
    ...
    auto c = matrix_library::operations::add(a, b);
...
TEST(OperationsAddTest, ShapeMismatchThrows)
{
    matrix_library::Tensor<int> a(std::vector<std::size_t>{2, 2});
    matrix_library::Tensor<int> b(std::vector<std::size_t>{3});

    EXPECT_THROW(matrix_library::operations::add(a, b), std::invalid_argument);
}

TEST(OperationsAddTest, EmptyTensors)
{
