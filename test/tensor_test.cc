...
TEST(TensorTest, InitializerListIndexingReadWrite) {
  matrix_library::Tensor<int> t(std::vector<std::size_t>{2, 3, 4});
  std::vector<int> vals(24);
  for (size_t i = 0; i < vals.size(); ++i)
  ...
  t[{0, 1, 2}] = 42;
  EXPECT_EQ((t[{0, 1, 2}]), 42);

  const matrix_library::Tensor<int> &ct = t;
  EXPECT_EQ((ct[{1, 0, 0}]), (t[{1, 0, 0}]));
