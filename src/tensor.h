...
private:
  ...
  // ``strides_[i]`` gives the number of elements to skip to move to the next
  // index in dimension ``i`` For example, for a tensor with shape {2, 3},
  // strides would be {3, 1} Using an indices vector {1, 2} would yield a flat
  // index of 1*3 + 2*1 = 5 Skip 3 elements to move to the next row, and 1
  // element to move to the next column

  // Another example for a tensor with shape {2, 3, 4}:
  // Strides would be {12, 4, 1}
  // Skip 12 elements to move to the next 3-D layer, 4 elements to move to the
  // next row, and 1 element to move to the next column Using an indices vector
  // {1, 2, 3} would yield a flat index of 1*12 + 2*4 + 3*1 = 23
